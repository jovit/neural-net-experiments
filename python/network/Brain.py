...
class Brain:
  ...
  def calculate_gradient_neuron(self, neuron, current_layer, expected_output):
    ...
    for synapse in neuron_obj.synapses:
        gradient = synapse.parent_neuron.get_activation_sigmoid() *
          neuron_obj.get_activation_sigmoid() *
          (1 - neuron_obj.get_activation_sigmoid()) *
          neuron_obj.relation_to_cost
  ...
  def apply_backwards_propagation(self, expected_output, layer=self.layers.length - 1):
    if layer == 0:
      return
